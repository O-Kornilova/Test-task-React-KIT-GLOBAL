...
export function PostList ({ initialPosts }: { initialPosts?: Post[] }) {
  const reduxPosts = useAppSelector(s => s.posts.items)
  console.log(
    'PostList render, reduxPosts:',
    reduxPosts.map(p => p.title)
  )
  const reduxStatus = useAppSelector(s => s.posts.status)
  const isLoading = reduxStatus === 'loading'
  const { filters } = useFilterStore()
  const openModal = useModalStore(s => s.openModal)

  const allPosts = reduxPosts.length > 0 ? reduxPosts : initialPosts ?? []
